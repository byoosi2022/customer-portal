...
async function fetchOrders() {
    try {
        const response = await fetch('/api/method/customer_portal.custom_api.orders.get_customer_sales_invoices');
        ...
        if (salesOrders) {
            ...
            salesOrders.forEach(order => {
                ...
                order.items.forEach(item => {
                    ...
                    totalOutstanding += order.outstanding_amount;
            ...
            amountPaid = totalAmount - totalOutstanding
            // After all orders are processed, add the total amount row
            const AmountPaidRow = document.createElement('tr');
            AmountPaidRow.innerHTML = `
                <td colspan="4" style="text-align:right;"><strong>Total Amount Paid</strong></td>
               <td colspan="2">
                    <span id="total-amount">₦${amountPaid.toLocaleString()}</span>
                </td>
                
            `;
            orderItemsContainer.appendChild(AmountPaidRow);

               // After all orders are processed, add the total amount row
            const OutstandingRow = document.createElement('tr');
            OutstandingRow.innerHTML = `
                <td colspan="4" style="text-align:right;"><strong>Total Outstanding</strong></td>
               <td colspan="2">
                    <span id="total-amount">₦${totalOutstanding.toLocaleString()}</span>
                </td>
                
            `;
            orderItemsContainer.appendChild(OutstandingRow);
